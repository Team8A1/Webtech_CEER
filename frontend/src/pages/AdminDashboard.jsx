...
const AdminDashboard = () => {
  ...
  const fetchLabIncharges = async () => {
    try {
      const response = await api.get('/lab/list');
      if (response.data.success) setLabInchargesData(response.data.data);
        ...
        <header className="bg-white/80 backdrop-blur-md border-b border-indigo-50/50 px-10 py-4 flex justify-between items-center sticky top-0 z-20 shadow-sm">
          <div>
            ...
            <p className="text-stone-500 text-sm mt-1 font-medium">
              ...
              {activeTab === 'staff' && 'Manage faculty guides and lab incharges'}
