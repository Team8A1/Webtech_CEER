...
function LabApprove() {
  ...
  useEffect(() => {
    ...
    const currentPendingCount = stored.filter(b => !b.labApproved).length

    // Play gentle reminder voice only once if there are pending BOMs
    if (currentPendingCount > 0 && !voicePlayedRef.current) {
      const utterance = new window.SpeechSynthesisUtterance('Gentle reminder, please approve the request')
      utterance.rate = 0.9
      utterance.pitch = 1.5
  ...
  const getFilteredBOMs = () => {
    if (filter === 'pending') return boms.filter(b => !b.labApproved)
    if (filter === 'approved') return boms.filter(b => b.labApproved)
    return boms
  }

  const filteredBoms = getFilteredBOMs()
  const pendingCount = boms.filter(b => !b.labApproved).length
