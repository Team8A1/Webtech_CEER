...
//           <div className="inline-block mb-0"></div>
...
//             {loading ? (
//               <div className="flex gap-10 items-center justify-center w-full h-[480px]">
//                 <div className="w-60 h-[380px] bg-slate-100 rounded-[2rem] animate-pulse hidden md:block" />
//                 <div className="w-[350px] h-[480px] bg-slate-200 rounded-[2rem] animate-pulse flex items-center justify-center text-slate-400">Loading Events...</div>
//                 <div className="w-60 h-[380px] bg-slate-100 rounded-[2rem] animate-pulse hidden md:block" />
//               </div>
//             ) : getVisibleCards().map(({ event, position, index }) => (
//               <div
//                 key={index}
//                 className={`group transition-all duration-700 ease-out ${position === 'center'
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="relative flex items-center justify-center min-h-[550px]">
          ...
          {!loading && recentEvents.length > 0 && (
            <button
              onClick={handlePrevious}
              className="absolute left-0 md:left-4 z-20 w-14 h-14 flex items-center justify-center bg-white/80 backdrop-blur-sm text-slate-800 hover:bg-white hover:text-red-900 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110"
              aria-label="Previous event"
            >
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 19l-7-7 7-7" />
          ...
          {/* Cards Container */}
          <div className="relative flex items-center justify-center w-full max-w-6xl h-[480px]">
            {loading ? (
              <div className="flex gap-10 items-center justify-center w-full">
                <div className="w-60 h-[380px] bg-slate-100 rounded-[2rem] animate-pulse hidden md:block" />
                <div className="w-[350px] h-[480px] bg-slate-200 rounded-[2rem] animate-pulse flex items-center justify-center text-slate-400 font-medium">Loading Events...</div>
                <div className="w-60 h-[380px] bg-slate-100 rounded-[2rem] animate-pulse hidden md:block" />
              </div>
            ) : (
              <div className="absolute inset-0 flex items-center justify-center gap-6 md:gap-10">
                {visibleCards.map(({ event, position, index }) => (
                  <div
                    key={`${index}-${position}`}
                    className={`absolute transition-all duration-700 ease-out ${position === 'left'
                        ? 'left-0 md:left-[10%] w-52 md:w-60 h-[340px] md:h-[380px] scale-90 opacity-50 hidden md:block'
                        : position === 'center'
                          ? 'left-1/2 -translate-x-1/2 w-80 md:w-[350px] h-[420px] md:h-[480px] scale-100 opacity-100 z-10'
                          : 'right-0 md:right-[10%] w-52 md:w-60 h-[340px] md:h-[380px] scale-90 opacity-50 hidden md:block'
                      }`}
                  >
                    <div className="group relative w-full h-full rounded-[2rem] overflow-hidden border border-slate-200/50 shadow-2xl hover:shadow-3xl transition-shadow duration-300 bg-white">
                      <img
                      ...
                      {position === 'center' && (
                        <>
                          {/* Animated Border Glow */}
                          <div className="absolute inset-0 rounded-[2rem] bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 opacity-0 group-hover:opacity-20 blur-xl transition-opacity duration-500" />

                          <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8 text-white">
                            <div className="flex items-center gap-2 mb-3">
                              <span className="px-4 py-1.5 bg-red-900/90 backdrop-blur-md rounded-full text-xs font-bold tracking-wide uppercase shadow-lg">
                                {event.category}
                              </span>
                              <div className="flex-1 h-px bg-gradient-to-r from-white/50 to-transparent" />
                            </div>
                            <h3 className="text-xl md:text-3xl font-bold mb-2 drop-shadow-lg line-clamp-2">
                              {event.title}
                            </h3>
                            <div className="flex items-center gap-2 text-sm text-slate-200">
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {!loading && recentEvents.length > 0 && (
            <button
              onClick={handleNext}
              className="absolute right-0 md:right-4 z-20 w-14 h-14 flex items-center justify-center bg-white/80 backdrop-blur-sm text-slate-800 hover:bg-white hover:text-red-900 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110"
              aria-label="Next event"
            >
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" />
